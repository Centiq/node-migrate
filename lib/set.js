...
Set.prototype._migrate = function(direction, migrationName, fn){
  ...
  newMigrations = takeUntil(newMigrations, migrationName, 'title');

  let migrationInMigrationsList = false;

  for (var i = 0; i < newMigrations.length; i++) {
    if(migrationInMigrationsList || newMigrations[i].title == migrationName){
      migrationInMigrationsList = true;
      continue;
    }
  }

  if(!migrationInMigrationsList){
    newMigrations = [];
  }
